...
async function tryLovableImage(prompt) {
  try {
    ...
    if (!r.ok) return null;
    const j = await r.json();
    const dataUri = j.choices?.[0]?.message?.images?.[0]?.image_url?.url || null;
...
async function tryAiWorkerImage(worker, prompt, minimal = false, productName = "Product", admin) {
  try {
    ...
    try {
      await admin.from("ef_logs").insert({
        fn: "ai-worker-image",
        step: "error",
        detail: `${worker.name}: ${msg.slice(0, 300)}`
      }).maybeSingle();
    } catch  {}
    return null;
  }
 ...
 */ export async function generateImageCascade(admin, prompt, tenantId, suffix = "gen", opts) {
  const debug = {
    geminiKeysTried: 0,
    workersTried: []
  };
  ...
  const { data: workers } = await admin.from("ai_workers").select("id, name, base_url").eq("is_active", true).eq("worker_type", "image").order("last_used_at", {
    ascending: true,
    nullsFirst: true
  });
  ...
  const { data: keys } = await admin.from("api_keys").select("id, api_key").eq("provider", "google_ai").eq("is_exhausted", false).order("last_used_at", {
    ascending: true,
    nullsFirst: true
  });
  ...
  if (allQuota) {
    ...
    const quickRetry = await Promise.all((keys || []).map((keyEntry)=>tryGoogleImage(keyEntry.api_key, prompt).catch(()=>null)));
    ...
    try {
      const r = await fetch("https://openrouter.ai/api/v1/chat/completions", {
        ...
        headers: {
          ...
          "Content-Type": "application/json",
          "HTTP-Referer": "https://smarthubly.pages.dev",
          "X-Title": "SmartHubly"
        },
        body: JSON.stringify({
          model: "google/gemini-2.5-flash-image",
  ...
  async function runWorkerPass(passLabel, minimal = false) {
    ...
    for(let i = 0; i < list.length; i += 2){
      ...
      for(let b = 0; b < batch.length; b++){
        ...
        if (out && out.bytes.length > 0) {
          await admin.from("ai_workers").update({
            last_used_at: new Date().toISOString(),
            is_exhausted: false
          }).eq("id", w.id);
          return {
  ...
  if (pass) {
    debug.workersTried = (debug.workersTried || []).concat([
      {
        id: pass.winner.id,
        name: pass.winner.name,
        http: 200
      }
    ]);
