// marketing-unified — Edge Function unificada (5 rotas).
// Roteamento por path: /post, /campaign, /refine, /review, /colors.
...
import { colors } from "./_routes/colors/index.ts";
...
const handlers: Record<string, (req: Request, body?: unknown) => Promise<Response>> = {
  ...
  "/review": review,
  "/colors": colors,
  "/": (req) => Promise.resolve(new Response(JSON.stringify({ error: "route_required", available: ["post", "campaign", "refine", "review", "colors"] }), { status: 400, headers: { "Content-Type": "application/json" } })),
};
