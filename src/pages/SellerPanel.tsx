...
type Item = { product_name: string; quantity: number; unit_price: number; line_total: number; net_profit: number; commission_amount: number };
type Order = { order_id: string; created_at: string; status: string; customer_name: string; customer_phone: string; customer_email: string | null; payment_method: string; total: number; discount_amount: number; items: Item[]; item_quantity: number; seller_sales: number; net_profit: number; seller_amount: number };
...
export default function SellerPanel() {
  ...
  const metrics = useMemo(() => {
    ...
    return {
      ...
      profit: valid.reduce((sum, o) => sum + Number(o.net_profit || 0), 0),
  ...
  return <main className="min-h-screen bg-muted/30 pb-12">
    ...
    <div className="mx-auto max-w-7xl space-y-6 px-4 py-6 md:px-8">
      <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
        {[['Pedidos', metrics.orders, ShoppingBag], ['Celulares vendidos', metrics.phones, Package], ['Vendas', money(metrics.sales), TrendingUp], ['Lucro líquido', money(metrics.profit), BadgeDollarSign], ['Você recebe', money(metrics.due), BadgeDollarSign]].map(([label, value, Icon]: any) => <div key={label} className="rounded-2xl border bg-card p-4 shadow-sm"><Icon className="mb-3 h-5 w-5 text-primary" /><p className="text-xs text-muted-foreground">{label}</p><p className="mt-1 text-xl font-bold">{value}</p>{label === 'Você recebe' && <p className="mt-1 text-xs text-green-600">20% do lucro positivo</p>}</div>)}
      </section>
      <section className="grid gap-6 lg:grid-cols-[1fr_2fr]">
        ...
        <div className="rounded-2xl border bg-card p-5"><h2 className="mb-3 flex items-center gap-2 font-semibold"><CalendarDays className="h-5 w-5 text-primary" /> Regras do repasse</h2><p className="text-sm text-muted-foreground">O valor recebido não é calculado sobre o faturamento bruto. Primeiro são descontados o custo do produto, taxa de checkout, entrega e demais custos registrados. Você recebe <strong className="text-foreground">20% somente do lucro líquido positivo</strong>. Pedidos cancelados não entram no total.</p><div className="mt-4 grid gap-3 sm:grid-cols-3"><div className="rounded-xl bg-muted p-3"><p className="text-xs text-muted-foreground">Produtos no catálogo</p><strong>{data.catalog.length}</strong></div><div className="rounded-xl bg-muted p-3"><p className="text-xs text-muted-foreground">Disponíveis agora</p><strong>{data.catalog.filter(p => p.in_stock !== false && (p.stock_quantity == null || Number(p.stock_quantity) > 0)).length}</strong></div><div className="rounded-xl bg-muted p-3"><p className="text-xs text-muted-foreground">Data atual</p><strong>{new Date().toLocaleDateString('pt-BR')}</strong></div></div></div>
      </section>
