...
const TenantCartDrawer = ({ tenant }: { tenant: Tenant }) => {
  ...
  const hasOnlinePayment = !isWhatsAppMode && !!((tenant as any).has_online_payment === true || (tenant as any).payment_provider === 'mercadopago' || (tenant as any).mercadopago_enabled === true || (tenant as any).payment_fee_pass_through_enabled === true || (tenant as any).store_mode === 'dropshipping' || !!(tenant as any).mercadopago_token || !!(tenant as any).pagbank_token || ((tenant as any).infinitepay_handle && (tenant as any).infinitepay_enabled));
