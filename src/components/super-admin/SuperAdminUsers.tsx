...
import { CheckCircle, XCircle, Clock, Shield, Store } from 'lucide-react';
...
const SuperAdminUsers = () => {
  ...
  const approve = useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ['super-admin-user-roles'] }),
  });
    ...
    <div className="space-y-6">
      <div>
        <h3 className="font-heading text-lg text-foreground flex items-center gap-2 mb-3">
          <Clock className="h-5 w-5 text-warning" /> Aguardando Aprovação ({pending.length})
        </h3>
        ...
        {pending.map(r => (
          <div key={r.id} className="flex items-center justify-between rounded-lg border border-border bg-card p-4 mb-2">
            <div className="min-w-0 flex-1">
              <p className="text-sm font-medium text-foreground truncate">{r.email || r.user_id.slice(0, 8) + '...'}</p>
              <p className="text-xs text-muted-foreground flex items-center gap-1">
                <Store className="h-3 w-3" /> {r.tenant_name} <span className="text-muted-foreground/60">/{r.tenant_slug}</span>
              </p>
              <p className="text-xs text-muted-foreground">Solicitou: {r.role}</p>
            </div>
        ...
        {approved.map(r => (
          <div key={r.id} className="flex items-center justify-between rounded-lg border border-border bg-card p-4 mb-2">
            <div className="min-w-0 flex-1">
              <p className="text-sm font-medium text-foreground truncate">{r.email || r.user_id.slice(0, 8) + '...'}</p>
              <p className="text-xs text-muted-foreground flex items-center gap-1">
                <Store className="h-3 w-3" /> {r.tenant_name} · {r.role}
              </p>
            </div>
            <button onClick={() => reject.mutate(r.id)} className="flex items-center gap-1 rounded-lg bg-red-600/20 text-red-400 px-3 py-1.5 text-xs font-medium hover:bg-red-600/30 ml-2 shrink-0">
