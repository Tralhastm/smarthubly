  ...
  return useMutation({
    mutationFn: async (product: Product) => {
      const { error } = await supabase.from('products').update({
        ...
        shipping_discount_enabled: Boolean((product as any).shipping_discount_enabled),
        shipping_discount_type: (product as any).shipping_discount_type ?? 'free',
        shipping_discount_value: Number((product as any).shipping_discount_value ?? 0),
