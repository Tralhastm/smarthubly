...
export async function new_order(req: Request, body?: unknown): Promise<Response> {
  try {
    const ct = req.headers.get("content-type") || "";
    const parsed: any = body ?? (ct.includes("application/json") ? await req.json() : {});
if (req.method === "OPTIONS") return new Response(null, { headers: corsHeaders });
  try {
    const { orderId, tenantId, customerName, total } = await req.json();
    if (!tenantId) {
      return new Response(JSON.stringify({ error: "tenantId required" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }

    const supabase = createClient(Deno.env.get("SUPABASE_URL")!, Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!);

    // Busca todas as inscrições push do tenant (lojista)
    const { data: subs } = await supabase.from("push_subscriptions").select("*").eq("tenant_id", tenantId);

    if (!subs || subs.length === 0) {
      return new Response(JSON.stringify({ sent: 0, message: "No tenant subscriptions" }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }

    // Busca slug pra criar URL de redirect
    ...
    const totalFmt = typeof total === 'number' ? `R$ ${total.toFixed(2)}` : '';
    const payload = JSON.stringify({
      title: `🔔 Novo pedido — ${tenant?.name || 'sua loja'}`,
      body: `${customerName || 'Cliente'} ${totalFmt ? `· ${totalFmt}` : ''}`.trim(),
      data: { url: `/loja/${slug}/admin`, orderId },
    });
    ...
    for (const sub of subs) {
      try {
        ...
        const response = await fetch(sub.endpoint, {
          method: "POST",
          headers: { ...vapidHeaders, ...encrypted.headers, TTL: "86400", Urgency: "high" },
          body: encrypted.body,
        });
    ...
    console.error("[unified:new-order] error", e);
