...
function stripSlug(path: string): string {
  ...
  // ou /ai-chat-unified/sofia-agent.
  ...
  // Se não achar o slug, tenta pegar o último segmento se houver mais de um
  if (parts.length > 1) {
...
export async function route(req: Request, handlers: Record<string, Handler>): Promise<Response> {
  ...
  const path = hdr ? (hdr.startsWith("/") ? hdr : "/" + hdr) : stripSlug(rawPath);
