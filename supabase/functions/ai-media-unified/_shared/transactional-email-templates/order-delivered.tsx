...
const Email = ({ customerName = '', tenantName = 'Loja', orderId = '' }: Props) => (
  <Html lang="pt-BR" dir="ltr">
    <Head />
    <Preview>Pedido #{orderId} entregue — obrigado!</Preview>
    <Body style={main}>
      <Container style={container}>
        <Section style={header}><Heading style={h1}>✅ Pedido entregue!</Heading></Section>
        <Section style={card}>
          <Text style={text}>Olá{customerName ? `, ${customerName}` : ''}! Seu pedido <b>#{orderId}</b> em <b>{tenantName}</b> foi entregue. Obrigado pela preferência! 💙</Text>
...
export const template = {
  component: Email,
  subject: (d: Record<string, any>) => `✅ Pedido #${d?.orderId ?? ''} entregue — obrigado!`,
