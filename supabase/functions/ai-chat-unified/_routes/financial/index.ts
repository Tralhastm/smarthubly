...
function buildSystemPrompt(businessData: any): string {
  ...
  const fmt = (n: any) => typeof n === "number" ? `R$ ${n.toFixed(2).replace(".", ",")}` : "—";
  const topProducts = Array.isArray(data.topProducts) && data.topProducts.length
    ? data.topProducts.slice(0, 5).map((p: any, i: number) =>
        `  ${i + 1}. ${p.name} — ${p.sales || 0} vendas — ${fmt(p.revenue)}`).join("\n")
...
function googleToOpenAIStream(googleStream: ReadableStream<Uint8Array>): ReadableStream<Uint8Array> {
...
async function tryGoogleStream(systemPrompt: string, messages: any[], keys: ApiKeyEntry[], supabase: any): Promise<Response | null> {
  ...
  for (const keyEntry of allKeys) {
    ...
    } catch (e) { console.error("Google stream failed:", e); continue; }
  }
  return null;
}
