...
export async function cindy_actions(req, body) {
  if (req.method === "OPTIONS") return new Response(null, {
    headers: corsHeaders
  });
  try {
    const ct = req.headers.get("content-type") || "";
    const payload = body ?? (ct.includes("application/json") ? await req.json().catch(()=>({})) : {});
    ...
    if (path === "/gen-post") {
      ...
      const { format = "post_dia", scope = "platform", tenantId, tone, extraContext, audience, generateImage = false, imageStyle, imagePrompt } = body;
      const validFormats = [
        "post_dia",
        "story",
        "bio",
      ...
      if (!validFormats.includes(format)) return j({
        error: "format_invalido"
      }, 400);
      if (scope === "tenant" && !tenantId) return j({
        error: "tenantId_obrigatorio"
      }, 400);
      ...
      if (!res.ok) {
        ...
        return j({
          error: "marketing_post_failed",
          detail: err?.error || String(res.status)
        }, 502);
      }
    ...
    if (path === "/reply-ticket") {
      ...
      if (!ticketId || !content || typeof content !== "string" || content.trim().length === 0) {
        return j({
          error: "ticketId_e_content_obrigatorios"
        }, 400);
      }
      const supabase = getSupabase();
      ...
      if (insErr) return j({
        error: "falha_ao_enviar_mensagem",
    ...
    if (path === "/list-tickets") {
      ...
      const supabase = getSupabase();
      const { data, error } = await supabase.from("support_tickets").select("id, subject, description, status, priority, tenant_id, category, created_at").in("status", [
        "open",
        "pending",
        "waiting"
      ]).order("created_at", {
      ...
      const lines = (data || []).map((t)=>`· [${t.id.slice(0, 8)}] ${t.subject} (${t.status || "open"}) | criado ${new Date(t.created_at).toLocaleString("pt-BR")}`);
      return j({
        ok: true,
        tickets: data || [],
