// finance-unified — Edge Function unificada (5 rotas).
// Roteamento por path: /sync-financeflow, /pull-financeflow, /estimate-quote, /insights, /auto-categorize.
...
const handlers: Record<string, (req: Request, body?: unknown) => Promise<Response>> = {
  ...
  "/": (req) => Promise.resolve(new Response(JSON.stringify({ error: "route_required", available: ["sync-financeflow", "pull-financeflow", "estimate-quote", "insights", "auto-categorize"] }), { status: 400, headers: { "Content-Type": "application/json" } })),
};
