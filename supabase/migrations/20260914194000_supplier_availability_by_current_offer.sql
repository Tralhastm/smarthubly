-- O arquivo arquivado é apenas auditoria da importação.
-- A verdade operacional é supplier_variant_offers.available:
-- cada fornecedor pode atualizar sua própria oferta e a reconciliação escolhe
-- automaticamente o menor custo entre todas as ofertas atuais.
...
BEGIN
  ...
  -- Não exige source_archive_id nem a validade do arquivo. Uma oferta ativa
  -- foi criada/reativada pela importação atual daquele fornecedor.
  WITH winner AS (
  ...
  -- Produtos sem variantes também usam somente ofertas atuais e o menor custo.
  WITH winner AS (
...
-- O checkout também deve validar a oferta atual, não a existência de arquivo.
...
DECLARE
  ...
  _variant_name text; _tenant_id text; _product_id uuid; _variant_id uuid;
