...
BEGIN
  ...
  WITH explicit_offer AS (
    ...
    FROM candidate
    -- Regra comercial: cor explicitamente confirmada vence qualquer fallback
    -- sem cor, mesmo quando o fallback tiver custo menor. Dentro do mesmo
    -- nível de precisão, vence o menor custo.
    ORDER BY product_variant_id, priority ASC, unit_cost ASC, md5(supplier_id || ':' || product_variant_id)
  ), target AS (
...
SELECT public.reconcile_supplier_catalog(id)
