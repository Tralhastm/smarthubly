ALTER TABLE public.tenants
  ADD COLUMN IF NOT EXISTS shipping_discount_enabled boolean NOT NULL DEFAULT true,
  ADD COLUMN IF NOT EXISTS shipping_discount_type text NOT NULL DEFAULT 'free',
  ADD COLUMN IF NOT EXISTS shipping_discount_value numeric(12,2) NOT NULL DEFAULT 0;
ALTER TABLE public.tenants DROP CONSTRAINT IF EXISTS tenants_shipping_discount_type_check;
ALTER TABLE public.tenants ADD CONSTRAINT tenants_shipping_discount_type_check CHECK (shipping_discount_type IN ('free', 'percent', 'fixed'));

ALTER TABLE public.products
  ADD COLUMN IF NOT EXISTS shipping_discount_enabled boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS shipping_discount_type text NOT NULL DEFAULT 'free',
  ADD COLUMN IF NOT EXISTS shipping_discount_value numeric(12,2) NOT NULL DEFAULT 0;
ALTER TABLE public.products DROP CONSTRAINT IF EXISTS products_shipping_discount_type_check;
ALTER TABLE public.products ADD CONSTRAINT products_shipping_discount_type_check CHECK (shipping_discount_type IN ('free', 'percent', 'fixed'));

-- As colunas novas ficam no final para preservar a ordem da view existente.
...
SELECT
  ...
  t.shipping_discount_enabled, t.shipping_discount_type, t.shipping_discount_value
