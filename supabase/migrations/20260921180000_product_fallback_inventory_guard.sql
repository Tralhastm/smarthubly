...
BEGIN
  IF TG_TABLE_NAME = 'products' THEN
    ...
    IF v_has_variants THEN
      SELECT EXISTS (
        ...
        JOIN public.supplier_variant_offers svo ON svo.product_variant_id = pv.id AND svo.tenant_id = NEW.tenant_id AND svo.available = true
        ...
        WHERE spp.available = true AND public.catalog_product_match_key(spp.product_name) = public.catalog_product_match_key(NEW.name)
      ) INTO v_valid_offer;
    ELSE
      SELECT EXISTS (
        SELECT 1 FROM public.supplier_product_prices spp
  ...
  SELECT EXISTS (
    ...
    JOIN public.supplier_product_prices spp ON spp.available = true AND public.catalog_product_match_key(spp.product_name) = public.catalog_product_match_key(p.name)
    JOIN public.supplier_catalog_archives a ON a.id = spp.source_archive_id AND a.expires_at > now()
...
DECLARE
  _id uuid; _payload jsonb; _cols text; _it jsonb; _fragment_pair record; _fragment_item jsonb;
  _supplier_id text; _fragment_items jsonb; _product_name text; _variant_name text; _tenant_id text;
  _product_id uuid; _variant_id uuid; _has_variants boolean; _valid_offer boolean;
BEGIN
  ...
  FOR _it IN SELECT * FROM jsonb_array_elements(coalesce(_items, '[]'::jsonb)) LOOP
    _product_name := NULLIF(trim(_it->>'product_name'),''); _variant_name := lower(trim(coalesce(_it->>'variant_name','')));
    SELECT p.id INTO _product_id FROM public.products p WHERE p.tenant_id=_tenant_id AND lower(trim(p.name))=lower(_product_name) LIMIT 1;
    ...
    IF _has_variants AND _variant_name <> '' THEN
      SELECT pv.id INTO _variant_id FROM public.product_variants pv WHERE pv.product_id=_product_id AND lower(trim(pv.name))=_variant_name AND pv.in_stock::text='true' LIMIT 1;
      IF _variant_id IS NULL THEN RAISE EXCEPTION 'variacao_indisponivel: % / %', _product_name, _it->>'variant_name'; END IF;
      SELECT EXISTS(SELECT 1 FROM public.supplier_variant_offers svo JOIN public.supplier_catalog_archives a ON a.id=svo.source_archive_id AND a.expires_at > now() WHERE svo.tenant_id=_tenant_id AND svo.product_variant_id=_variant_id AND svo.available=true)
        OR EXISTS(SELECT 1 FROM public.products p JOIN public.supplier_product_prices spp ON spp.available=true AND public.catalog_product_match_key(spp.product_name)=public.catalog_product_match_key(p.name) JOIN public.supplier_catalog_archives a ON a.id=spp.source_archive_id AND a.expires_at > now() WHERE p.id=_product_id) INTO _valid_offer;
    ELSE
      SELECT EXISTS(SELECT 1 FROM public.supplier_product_prices spp JOIN public.supplier_catalog_archives a ON a.id=spp.source_archive_id AND a.expires_at > now() JOIN public.suppliers s ON s.id=spp.supplier_id AND s.tenant_id=_tenant_id WHERE spp.available=true AND public.catalog_product_match_key(spp.product_name)=public.catalog_product_match_key(_product_name)) INTO _valid_offer;
    END IF;
  ...
  FOR _it IN SELECT * FROM jsonb_array_elements(coalesce(_items,'[]'::jsonb)) LOOP
    ...
    SELECT string_agg(format('%I',k),',') INTO _cols FROM jsonb_object_keys(_payload) k;
    EXECUTE format('INSERT INTO public.order_items (%s) SELECT %s FROM jsonb_populate_record(null::public.order_items,$1)',_cols) USING _payload;
  END LOOP;
  FOR _fragment_pair IN SELECT key AS supplier_id,value AS items FROM jsonb_each(COALESCE(_order->'metadata'->'fragmentation_map','{}'::jsonb)) LOOP
    ...
    INSERT INTO public.order_fragments(order_id,tenant_id,supplier_id,items,total,status) VALUES (_id::text,_tenant_id,_supplier_id,_fragment_items,COALESCE((SELECT SUM(COALESCE((item->>'quantity')::numeric,0)*COALESCE((item->>'product_price')::numeric,0)) FROM jsonb_array_elements(_fragment_items) AS x(item)),0),_order->>'status');
    FOR _fragment_item IN SELECT * FROM jsonb_array_elements(_fragment_items) LOOP
      _product_name := COALESCE(_fragment_item->'product'->>'name',_fragment_item->>'product_name'); _variant_name := COALESCE(_fragment_item->>'variantName',_fragment_item->>'variant_name','');
