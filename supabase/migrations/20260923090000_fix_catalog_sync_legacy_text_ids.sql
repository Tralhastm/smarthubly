-- The legacy catalog schema stores tenant/product/variant identifiers as text.
-- Keep the RPC argument text-compatible with those columns and cast only for auth helpers.
DROP FUNCTION IF EXISTS public.sync_supplier_variant_inventory(uuid);

CREATE OR REPLACE FUNCTION public.sync_supplier_variant_inventory(_tenant_id text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  IF NOT (
    public.has_role(auth.uid(), 'admin'::public.app_role, _tenant_id::uuid)
...
REVOKE ALL ON FUNCTION public.sync_supplier_variant_inventory(text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.sync_supplier_variant_inventory(text) TO authenticated;
