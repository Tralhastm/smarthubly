DROP FUNCTION IF EXISTS public.assign_order_driver_by_supplier_token(text, uuid, uuid);
  ...
  WHERE id = _order_id
    ...
    AND (
      ...
      OR EXISTS (
        ...
        WHERE f.order_id = orders.id AND f.supplier_id::text = v_supplier.id::text
      )
      OR EXISTS (
        SELECT 1 FROM public.order_items oi
        WHERE oi.order_id = orders.id AND oi.supplier_id::text = v_supplier.id::text
      )
    )
  FOR UPDATE;
...
REVOKE ALL ON FUNCTION public.assign_order_driver_by_supplier_token(text, uuid, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.assign_order_driver_by_supplier_token(text, uuid, text) TO anon, authenticated;
