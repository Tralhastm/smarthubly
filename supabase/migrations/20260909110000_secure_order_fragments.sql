...
BEGIN
  ...
  -- O checkout público não pode inserir diretamente em order_fragments por RLS.
  -- A fragmentation_map chega dentro do pedido e é materializada aqui, em contexto
  -- SECURITY DEFINER, junto com o fornecedor de cada item.
  ...
  LOOP
    ...
    _fragment_total := COALESCE((
      SELECT SUM(COALESCE((item->>'quantity')::numeric, 0) * COALESCE((item->>'product_price')::numeric, 0))
    ...
    VALUES (_id::text, _order->>'tenant_id', _supplier_id, _fragment_items, _order->>'status');
