...
BEGIN
  ...
  WITH candidate AS (
    SELECT svo.product_variant_id, svo.supplier_id, svo.unit_cost
    ...
    FROM candidate
    ORDER BY product_variant_id, unit_cost ASC, md5(supplier_id||':'||product_variant_id)
  ), target AS (
    SELECT pv.id,p.price,pv.price_delta,pv.allow_loss,pv.suggested_price,w.supplier_id AS winner_supplier_id,w.unit_cost AS winner_cost
  ...
  UPDATE public.product_variants pv SET
    supplier_id=t.winner_supplier_id,cost_price=t.winner_cost,
    price_source=CASE WHEN t.winner_supplier_id IS NULL THEN NULL ELSE 'supplier_offer' END,
    in_stock=CASE WHEN t.winner_supplier_id IS NULL THEN 'false' WHEN t.allow_loss THEN 'true' ELSE CASE WHEN (COALESCE(t.price+COALESCE(t.price_delta,0),t.price,0)-(COALESCE(t.price+COALESCE(t.price_delta,0),t.price,0)*0.0499)-50-10-t.winner_cost)>=0 THEN 'true' ELSE 'false' END END,
    needs_price_review=CASE WHEN t.winner_supplier_id IS NULL THEN false ELSE COALESCE(t.suggested_price,0)<=0 END,updated_at=now()
  FROM target t WHERE pv.id=t.id;
  GET DIAGNOSTICS v_variants=ROW_COUNT;
  UPDATE public.products p SET supplier_id=NULL,in_stock=EXISTS(SELECT 1 FROM public.product_variants pv WHERE pv.product_id=p.id AND pv.in_stock::text='true'),updated_at=now()
  ...
  WITH winner AS (
    ...
    FROM public.products p JOIN public.supplier_product_prices spp ON spp.available=true AND public.catalog_product_match_key(spp.product_name)=public.catalog_product_match_key(p.name)
  ...
  UPDATE public.products p SET supplier_id=w.supplier_id,original_price=w.unit_price,in_stock=CASE WHEN p.allow_loss THEN true ELSE (COALESCE(p.price,0)-(COALESCE(p.price,0)*0.0499)-50-10-w.unit_price)>=0 END,updated_at=now() FROM winner w WHERE p.id=w.product_id;
...
SELECT public.reconcile_supplier_catalog(id) FROM public.suppliers WHERE tenant_id='490d0a8a-353d-4db8-ab53-2132adb60b21';
